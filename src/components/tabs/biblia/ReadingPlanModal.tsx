...
export function ReadingPlanModal({ isOpen, onClose, onReadChapter }: ReadingPlanModalProps) {
  ...
  const effectiveView: ModalView = activePlan ? (view === "list" ? "active" : view) : view;
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-4"
        ...
        >
          <motion.div
            ...
            className="w-full sm:max-w-lg bg-white rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col max-h-[92vh]"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-5 border-b border-[#F3F4F6] flex-shrink-0">
              <div className="flex items-center gap-3">
                {(view === "detail" || (view !== "active" && effectiveView !== "active")) && view !== "list" && (
                  <button
                    onClick={() => { setView("list"); setPreviewPlan(null); }}
                ...
                <h2 className="text-lg font-bold text-[#1F2937]">
                  {effectiveView === "active" ? currentPlan?.title ?? "Plano Ativo" : view === "detail" ? previewPlan?.title : "Planos de Leitura"}
                </h2>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              <AnimatePresence mode="wait">
                ...
                {/* ── LISTA DE PLANOS ── */}
                {(effectiveView === "list" || (view === "list" && !activePlan)) && (
                  <motion.div
                    key="list"
                  ...
                  >
                    <p className="text-sm text-[#6B7280]">
                      Leia a Bíblia com consistência. Ganhe <span className="font-semibold text-violet-600">+25 XP</span> por capítulo lido.
                    </p>
                    ...
                    {READING_PLANS.map((plan) => (
                      <motion.button
                        ...
                        className="w-full p-4 rounded-2xl border border-[#E5E7EB] hover:border-gray-300 text-left transition-all group hover:shadow-md"
                      >
                        <div className="flex items-start gap-4">
                          <div className={cn(
                            "w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 text-2xl shadow-md group-hover:scale-105 transition-transform",
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold text-white" style={{ backgroundColor: plan.accentColor }}>
                                {plan.subtitle}
                              </span>
                            </div>
                            <p className="text-xs text-[#6B7280] leading-relaxed line-clamp-2">{plan.description}</p>
                ...
                {/* ── DETALHE DO PLANO (preview antes de iniciar) ── */}
                {view === "detail" && previewPlan && (
                  <motion.div
                    key="detail"
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                  ...
                  >
                    {/* Hero */}
                    <div className={cn("rounded-2xl p-5 bg-gradient-to-br text-white", previewPlan.color)}>
                      ...
                      <div className="flex items-center gap-4 mt-4 pt-4 border-t border-white/20">
                        <div className="text-center">
                          <p className="text-xl font-bold">{previewPlan.duration}</p>
                          <p className="text-white/70 text-xs">dias</p>
                        </div>
                        <div className="text-center">
                          <p className="text-xl font-bold">{previewPlan.days.reduce((s, d) => s + d.readings.length, 0)}</p>
                        ...
                        {previewPlan.days.slice(0, 5).map((day) => (
                          <div key={day.day} className="flex items-center gap-3 p-3 rounded-xl bg-[#F9FAFB]">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-[#9CA3AF]">{day.readings.map(r => `${r.bookName} ${r.chapter}`).join(", ")}</p>
                            </div>
                          </div>
                        ))}
                        {previewPlan.days.length > 5 && (
                          <p className="text-xs text-center text-[#9CA3AF] py-1">+ {previewPlan.days.length - 5} dias restantes</p>
                        )}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={cn("w-full py-4 rounded-2xl text-white font-bold text-base bg-gradient-to-r shadow-lg hover:shadow-xl transition-shadow", previewPlan.color)}
                    >
                      Começar Agora
                    </button>
                  </motion.div>
                ...
                {effectiveView === "active" && currentPlan && activePlan && (
                  ...
                  >
                    {/* Progresso geral */}
                    <div className={cn("rounded-2xl p-4 bg-gradient-to-br text-white", currentPlan.color)}>
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <p className="text-white/70 text-xs">Dia atual</p>
                          <p className="text-2xl font-bold">{currentDay} / {currentPlan.duration}</p>
                        </div>
                    ...
                    {todayReadings && (
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h4 className="font-semibold text-[#374151] text-sm">Hoje — {todayReadings.title}</h4>
                        </div>
                        <div className="space-y-2">
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  <p className={cn("font-medium text-sm", read ? "text-emerald-700 line-through" : "text-[#374151]")}>
                                ...
                                {!read && <ChevronRight className="w-4 h-4 text-[#D1D5DB]" />}
                                {read && <Star className="w-4 h-4 text-amber-400" />}
                              </motion.button>
                            );
                          })}
                        </div>
                    ...
                    {activePlan.isCompleted && (
                      <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-center">
                        ...
                        <p className="text-sm text-[#6B7280] mt-1">Parabéns! Você completou o {currentPlan.title}.</p>
                      </div>
                    )}
                    ...
                    <div className="flex gap-3 pb-2">
                      {activePlan.isCompleted ? (
                        ...
                        <button
                          ...
                          className="flex-1 py-3 rounded-xl border border-red-200 text-red-500 font-medium text-sm hover:bg-red-50 transition-colors"
                        >
                          Abandonar plano
                        </button>
                      )}
